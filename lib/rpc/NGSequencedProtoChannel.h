...
@interface NGSequencedProtoChannel : NSObject {
	NSMutableDictionary *_expectedMessages;
	NGHost *host;
	NGProtoCallback *callback;
	
	NSInputStream *_inputStream;
	NSOutputStream *_outputStream;
	PBCodedInputStream *_pbInputStream;
	PBCodedOutputStream *_pbOutputStream;
}

@property (retain) NGHost *host;
@property (retain) NGProtoCallback *callback;
